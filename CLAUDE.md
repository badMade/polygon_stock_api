...
1. Check the last successful batch:
```bash
cat checkpoint.json
```

2. Modify script to resume from that batch:
```python
...
3. **Compression**: Compress batch files to save disk space
4. **Database**: Use local SQLite before Notion upload
...
3. Verify batch files are valid JSON
